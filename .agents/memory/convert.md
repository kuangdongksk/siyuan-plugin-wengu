...
- **洗牌同步改写解析字母**（Issue #123，20260915；⚠️ **已于 #176 收窄时
  ...
  B 错误」），`shuffleGroup` 原先只重写答案字母 → 解析字母全部失配。当年
  按**同一条字母映射**（旧字母 → 新字母，与 `newRun` 共用同一套 `order`，
  禁分头重算）改写解析类部件里的**独立裸字母词符**。
  ⚠️ **现况：这条改写整体退役**——`shuffleDraftOptions` 只重排选项 + 重写
  答案字母，解析文本一律原样；展示层 `CardDisplayShuffle` 同口径。下面是
  历史口径纪要（判据部分**已搬到 `LetterRefs`/`OptGroups` 供落库专用**）。
...
- **20260915 大转向：库=死形态，洗牌挪去展示层**（Issue #131，与 #123 同根
  ...
  「库是死的、洗牌是函数、解析无选项字母」，四块咬合：
    ...
    - **写库洗牌全部撤除**：`ConvertSegment`/`GenQuestion`/`RegenDialog` 三处
      ...
      ⚠️ **`OptionShuffle` 收窄到只剩判据**（Issue #214，20260922）：洗牌实现
      `shuffleGroup`/`collectGroups`、入口 `shuffleDraftOptions`、挤行拆行
      `unpackPackedSingle` 与整份专用单测随存量兼容口径一起删除（原「实现
      与单测保留给存量数据」的口径作废）。保留下来的 `POSITION_SENSITIVE`
      **原地不动**（`BankRepair` / `CardDisplayShuffle` 两处 import 零改动）；
      挤行拆行早已另家 `draft/OptionUnpack.unpackPackedOptions`（三条落库链
      消费）。死形态下若在生成链上恢复洗牌 = 把死形态又洗乱，别再接线。
      ⚠️ **判据落点已接出**（#176）到 `draft/LetterRefs.ts`（词符/保护区/
      所有格/英文正文词/引用前缀）与 `draft/OptGroups.ts`（选项组分组/
      字母表/长文本截断）——**落库规范化专用**，改口径只改这两个文件，
      别在调用侧另起正则。展示层**已不再消费**它们（见下条）。
...
unpackPackedOptions(d)))`）：`SetWriter.append` / `GenQuestion.genWithVerify`
      ...
      口径要点：
        ...
        - 「字母 + 全文」形态把标签（`B.` / `（B）`）与全文**一并吃掉**，
          ...
          （`「文本」 正确`）、标点不归引用（`A，B 均错`）；
        - **只堵新流量、不治存量**（#176 收窄，20260919）：存量解析里的
          字母**不做迁移**——用户重新转换一次即消化（重转产物过本道）。
          展示层也不再兜底（`CardDisplayShuffle` 只重映射 `answer`），
          存量换序后解析字母仍指库内原序位，属**已知接受态**。
