...
- **填空语义判等复核（Issue #187，20260921；规划稿 §三 B1）**：本地精确判分
  **失配**时问 Jev 一次「语义等价吗」，**≥0.9 才翻对**并标「Jev 判同」——
  ...
  （需要宿主与卡面的那层：在途结果行、判同后卡面/题号、视图三件组装）。
    ...
    - **标记必须能活到收卷/恢复**：`revealCard` 按入参或会话结果补画
      「（Jev 判同）」——只在提交那一瞬写结果行的话，收卷重画即丢。
    - **去重表只存「已判同」的键**（题 + 归一化作答；`globalThis` 单例）：
      判错/不确定不入表，同一题同一答不重问，判错后换答仍可复核。
    ...
    - ⚠️ **`quiz/index.ts` 行长额度＝上限（561）**：本单把复核三件挂进既有
      ...
      落点照此走，别往视图上堆成员。
    - 三个测试文件分工：`GapJudge.test.ts`（阈值/闸/失败回落/入账，mock 传输）、
      `GapReview.test.ts`（接线：谁被调、卡面怎么写）、`GapNoKey.test.ts`
      （无 key 零变化，判对/判错两路都盯结果行）。
