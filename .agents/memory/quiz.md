...
- **展示层选项洗牌**（Issue #131，20260915）：消剧透从「生成期洗牌」搬到
  ...
  原文位置、解析不含任何选项字母，见 convert 域）。
    ...
    - ⚠️ **解析里的字母引用同步改写**（Issue #176，20260918 真机报障；
      `CardDisplayShuffle.remapRefs`）：#131 的冻结口径「库内解析不含任何
      选项字母」**实际没守住**——工作区 bank 实查 841 道 single/multiple 里
      **835 道（99.3%）**解析带字母引用（AI 无视 `〔opt:X〕` 标记约定，直接
      写「B. 全文」或裸字母）。洗牌只重映射 `answer` 时，解析里的字母仍指
      **库内原序位**，换序后全体失配（真机截图：卡面 B 位显示甲、解析说
      「B. 乙」正确）。故洗选项组时用**同一份 order 映射**改写该组**解析**
      里的引用字母（治存量，纯读侧零迁移）。
        - ⚠️ **题干（`stemMd`）不在改写范围**（20260918 复核更正，此前此处
          与模块注释都写作「解析/题干」，实现只回填 `solutionMd`）：题干里的
          独立字母多为**实体名**（「甲、乙」「A、B 两点」「A 组」），照映射搬
          会把实体指代改错位，而验收只点解析；要接得先在真机语料上量误伤面。
        - ⚠️ **steps 题级 `solutionMd` 同样不改**（同次复核更正，模块注释原写
          「按顶层组口径改写」，实现里没有这段）：各步选项组**各有自己的字母
          映射**，题级解析里的「B」指哪一步的 B 无从判定，猜错即搬错指代。
        - 词符口径/数学与代码保护区/所有格 + 英文正文词（`Plan A`）/引用前缀
          全复用 `convert/.../draft/LetterRefs`（唯一判据落点）与
          `OptGroups.remapQuotedHead`；两处各写一套正则必然漂移
          （#176 自验实录：一套认 `Plan A`、另一套不认）。
        - **字母确为该组真实选项才映射**（超范围字母 E/F 不动）；
          字母表按**渲染序文本**取（`OptGroups.collectOptionGroups` 认
          `option*`/`step-k-option*`，含挤行拆分与标签剥离）——禁用原始
          markdown 数组（挤行时项数不同）。
        - ⚠️ **两次踩坑，都是顺序/自洽问题**：
            1. 引用前缀（`「A proposal…」` 里的 A）与「引用正文的句首字母」
               是**同一个字符**——先改前缀再改词符（`remapQuotedHead` 为预
               处理），反了会被映射两次（A→C→B 反向跳一个位次）；
            2. `rewriteLetters` 的跳过分支**必须原样回补**命中（受保护区/
               引用前缀/所有格），而原答案字母必然出现在解析里——两者混在
               同一输出流会**长度错位累积**（真机：解析少一个字符）。故
               「跳过」的判据只认文本位置，与字母本身无关。
        - `remapQuotedHead` 必须**全局**替换（不是只认文本开头）：一道题里
          多条引用（`「B. 甲」正确，「A. 乙」错误`）都要跟着映射。
