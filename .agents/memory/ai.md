...
- **AI 会话登记与管理工作区面板**（20260831）：
    ...
    - 判题/转换/检测/标签/路由/出题/单词复盘等带 track 的调用自动登记，面板
      回看完整轮次与产出，失败可重试。
    - **Jev 判定也登记**（Issue #201，20260921；**推翻 #183 的「不登记」决策**）：
      判定不是对话、没有生成式产出，但用户要能回看「问了什么、判成什么、为什么
      失败」。落点收口在 `ai/jev/track.ts`：`judgeJev` opts 加 optional
      `track{title, group?}`，**kind 固定 `"jev"`**（id 由 track 模块生成，不是
      内核 sessionID——判定没有会话）。六落点各传动作上下文作 title（质检/预筛带
      文档名、增量变更带题集名、填空判同带题号、单词判档带词数、同义判定带对数），
      同一次动作的多批挂同组（组机制数据层本就有，树上自动合并主题）。
      **登记簿 schema 零新增**：user 轮=state 摘要（单行化 + 600 字封顶）、
      ai 轮=类型化答案摘要（`qN：是/否 0.93` / `choice（confidence）` / `N 档`），
      `model` 填 `JEV_MODEL`——`begin/succeed/fail` 现有字段够用。
      ⚠️ **两个守门**：①**jev 记录不出重试钮**（`core/SessionDetail` 按
      `isJevRecord` 排除）——重试走 `agentChatContinued`，那是**生成式专属**
      通道，jev 没续聊、出了钮也跑不通；判定的重来办法是原动作重跑。
      ②**仍不进全局在途闸**（#183 红线第 2 条不变）：登记只挂登记簿，
      `slotGate` 一行不碰、不占生成式槽位。无 key 时 `isJevEnabled` 为假 ⇒
      零请求零登记（零行为变化）。
      ⚠️ **行名取词踩坑（复核补）**：A3 落点用了 `aiTitleJevChange` 而中英字典
      都没这一键 —— 取词 `i18n[k] || k` 缺键不报错，面板那条记录的行名就是
      字面量 `aiTitleJevChange`（证据：既有三条门禁都只管「字典有而代码无」）。
      现补了**反向门禁**（`i18n/dict.test.ts` 扫全仓非测试源文件的取词实参 +
      `aiTitle*/aiKind*/aiFlow*` 族字面量），口径落在 `docs/design-spec.md §8.1`。
...
- **整页不滚动（Issue #96，20260915；20260916 Issue #146 恢复）**：
  ...
  ⚠️ **行轨道数必须等于 areas 行数**（20260916 复查实测的真回归点）：折单列把
  areas 从 2 行换成 3 行，而 `grid-template-rows` 若仍只声明 2 条轨道（继承基础
  片那两条），**第 3 行退化成隐式 `auto`** ⇒ 详情行按内容长到卡高、树行被挤成 0
  （900×800 无头 Chromium 实测：树列只剩 21px 内衬、可见行数 0，等于把树列与它
  那扇滚动窗一起撤掉）。也**不能**退回 `auto auto minmax(0,1fr)`（#96 当年那版，
  树行 auto 长满后把详情压成 0）。四种形态（长树/长详情/两边都长/一边为空）实测
  只有「两行各 `minmax(0,1fr)`」都对；锁在 `core/AiPanelScrollChain.test.ts`。
...
- ⚠️ **叶行行尾注记是「间隙期形态」**（见 SessionPanelApp 头注释）：S5 要求删
  ...
  它的时间）。故未选中时出记录时刻注记（`aiRowMeta`，i18n 单模板），选中即整行
  让位给详情；删除钮仍 hover 才显。等 `data/AiSessions.ts` 存下逐轮时刻后，
  按 S5/S6 的终态收敛（时间戳只留详情日志列）。
    - ⚠️ **#170（20260918）把间隙期形态收窄了两档**（真机报障：窄侧栏下叶子行
      「点 + 名 + 注记 + 徽标」四件塞满，稿里只有三件）：
      ① **运行中行整条不渲染注记**——判据取**视图**的 `lv.spin`（与徽标转圈同源），
      **不许**拿 `lv.dotCls === 'run'`（色名=视图词表）或 `rec.status === 'running'`
      （状态词=数据词表）：#92 的踩坑正是两套词表混用（只有 done 重叠，run 静默失效）；
