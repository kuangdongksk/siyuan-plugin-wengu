...
| 修法     | 树列补：`background:var(--b3-theme-background)`（surface-3 落点）、`border-right:1px solid var(--b3-border-color)`、`padding:8px 0 12px`；`padding-right:2px` 删（右边线即界）；删裸 `max-height:calc(100vh - 260px)`。⚠️ **滚动归属不在本条裁决范围内**：本条只定树列的**静态视觉四件**（凹槽底/右边线/padding/min-width）。#129 曾按稿把两列都改成「不设滚动窗」⇒ 20260916 出全屏滚动条（Issue #146），现行口径＝**两列各自列内滚**（规范 §12「整页不滚动」，宿主档 + 高度链见 `scss/aipanel.scss` / `scss/rail.scss`） |
