...
**✅ 反向门禁已落地**（Issue #201 复核，20260921；`src/i18n/dict.test.ts`）：
上面三条只管「字典有、代码没用」（§8.4 死键），**反方向「代码用了、字典没有」
原先无闸**——而它更致命：取词 `i18n[k] || k` 缺键**不报错**，把键名当正文
渲染给用户。#201 实测即此形态（A3 落点用了 `aiTitleJevChange`，中英字典都没
这一键，面板行名就是字面量 `aiTitleJevChange`；三条既有门禁逐条都扫不到它）。
现扫全仓**非测试**源文件的取词实参 + 命名分域前缀族字面量，缺键即红（含
「扫描面非空」自检防正则失效的假绿）。
...
- **技术要点（缺一条链就整页照滚）**：
    ...
    3. 主区经 `--fit` 档改写（`.wengu-ws-main--fit`：flex 列 + `overflow:hidden`），
       **开/关在面板挂载/卸载处配对**，且**只动本面板那一份骨架、禁 document
       级全选**（判定 `workspaceFits` / `fitTargetOf` / `toggleFit` 在
       `ai/core/PanelFit.ts`，带单测）；
    4. 内滚窗落在**列**上，列加 `min-height:0` 与 `scrollbar-gutter: stable`；
    5. 弹窗长内容同口径（`max-height` 封顶 + 内容区自滚）。
- ⚠️ **档位按工作区白名单开**：当前只有 AI 会话面板挂 `--fit`（`FIT_WORKSPACES`
  在 `ai/core/PanelFit.ts`）。**开了档而不收内滚** ⇒ 内容被 `overflow:hidden`
  切掉且滚不到，比「整页滚」更坏；**挂了链而不开档** ⇒ 卡在可滚主区里拿不到
  「剩余高」，内滚窗不出现（本项目 20260916 就是后者：`#129` 对稿还原把高度链
  连挂载开关一起删了 ⇒ AI 会话工作区又出全屏滚动条，Issue #146 恢复）。
  新增收内滚的面板要同时改白名单与挂载处的配对开关。
- **例外（有稿约束者按稿走，并在下表登记）**：判据「**看稿不看本规范条文**」。

    | 面板                      | 状态                                              |
    | ------------------------- | ------------------------------------------------- |
    | AI 会话（ai）             | **本条生效**（20260916 起撤销 `#129` 的例外登记） |
    ...
    不回退成页面级滚动。
    **两块硬骨头**（都是「清单与自己那扇滚动窗一起被撤掉」的形态，被否决）：
    ①给树列定高上限（40vh 上限 = 内滚窗失效）；②**行轨道数与 areas 行数不等**——
    折单列把 areas 从 2 行换成 3 行，`grid-template-rows` 若仍只声明 2 条轨道，
    第 3 行退化成**隐式 `auto`** ⇒ 详情行按内容长到卡高、树行被挤成 0（本项目
    20260916 复查实测；锁在 `ai/core/AiPanelScrollChain.test.ts`）。**改 areas
    行数必须同步改 `grid-template-rows` 轨道数**。
