...
### 第一期：路由结果按题哈希缓存（小改动，独立收益）

- 缓存键：`questionHash(题 kramdown)` + 知识索引代数（根集合 +
  `buildKnowledgeIndex` 产出章数/小节数的指纹）+ 模型 id。
- 缓存值：路由命中的小节引用列表（`{id,title}[]`）。
- 存放：`saveData("route-cache")`，容量上限（如 2000 条 LRU）。
- 命中即跳过两次 AI 调用（章+小节），未命中照常路由并写入。
- 三处弹窗共用；索引代数变了（增删知识根/章节数量变化）整体失效
  重建——宁漏勿错，不复用过期路由。
