...
- **结构切块**：`structuralChunks`——标题行（h1~h6）为边界，键=祖先
  ...
  切块（ConvertDetect）与切块无关，不动。**20260903 演进（有意的一次
  性漂移，非格式违约）**：答案类子节（键末段=答案/解答/题解/参考答案/
  答案与解析，~n 消歧放行）并入父题块——题解书题干块与答案块独立成
  块会让 AI 各转一题（原文一题转换后 1.7 题，真机实测），并块后一题
  一答；键格式与确定性不变，存量指纹按三态分类走用户可控的
  变更/消失（弹窗过目后重生成/删除），非静默漂移。
...
- **入口优先级**（20260903 修订：**带指纹即增量**）：题集记录带
  src-hash 就走增量（此时清掉该源的续跑记录——指纹自愈已取代偏移
  断点，陈旧记录留着会让「继续生成」重复生成已补块）；无 src-hash
  的旧版题集才走续跑记录/整卷重转——升级路径不产生重复题。全部
  相同直接收口「源未变化」零成本。**检测必过目**（20260903）：先弹
  检测摘要（源共几块/已入库几块/本次待处理几块）再执行，省费模式也
  不再静默直跑。
- **中止自愈**：增量执行中止后已追加块自带指纹，重跑分类即「相同」
  跳过，无需续跑记录；失败/中止路径同样跑题库幂等重扫
  （refreshDocFor：追加块入 records、删除块出 records）。注意
  **零产物块无指纹**（AI 判定无可转内容如例题/引言——不入库即无
  trace），每次重导都会重算为「新增」重发 AI；纯标题块在入口前置
  滤掉，零产物计数（IncrementOutcome.empty）在终态点明。20260903
  真机踩坑：题解书的「答案」节独立成块，被生成 prompt 的例题筛选
  整批误跳过——prompt 已加例外（习题册答案/解答区照转，题干由解答
  还原）。
- 省费模式设置项 `convertKeepOld`（设置→AI 转换；20260903 起=不出
  逐块清单、只出检测摘要确认，口径仍为变更/消失全保留只补新增）。
  新块**串行**生成（增量通常只几块，不复制整卷
...
## 四、20260910 变更：整卷转换改逐段自推进（后加一层分片并行），增量边界随之划定

整卷转换不再预切块（旧的 `structuralChunks` 按标题链切块已从主流程移除，
见 AGENTS.md「逐段自推进」）：改为自推进游标循环，AI 每批从窗口里读一段、
出题并回报 `@@TO` 定位行，游标按该行推进——**批边界由 AI 按题目边界决定**。

同日追加的**分片并行**（见 AGENTS.md 同名条目）把「任务并行单元」与「批边界」
拆开：外层 `planShards` 把源卷切成若干片（片间并行、片内仍由 AI 自推进），
切点按可靠性分级择取（标题行 > 超级块/分割线 > 题号行），产物经片序闸门按片序
落库，因此**题库里恒为按源顺序的连续前缀**、续跑断点仍是单游标。对本节口径的
影响：**没有影响**——批边界的决定权仍在 AI，`A:` 前缀与 src-hash 的批区间口径
逐字不变。

对增量的影响与口径：

- 逐段产物写入的记录 src-key 形如 `A:<区间起点偏移>`、src-hash=该批区间原文
  指纹（`questionHash`）。它是**批区间口径**，不是段落内容口径。分片并行下
  偏移仍是**全文绝对偏移**（片只是窗口上限），故键无歧义。
- 批边界不可复现（同一源文档两次转换，AI 分批点不会逐字一致）→ 逐段题集
  **没有确定性分类依据**：DocOps 见 `A:` 前缀即跳过增量三态分类，重新导入
  按整卷重转（有续跑记录仍接着断点续写同一题集，浮层告知）。
- `structuralChunks`/`classifyChunks`/`ConvertIncrement` 与 `H:` 键的存量
  题集增量能力**保持原样**（冻结清单里 srcKey 键格式的约定不变，两套键并存）。
- 后续若要给逐段题集补回增量能力，可行路径有两条：(a)「重导时先跑一轮轻量
  边界扫描（AI 只输出边界锚点、不出题）→ 与既有记录比锚点做三态」；(b) 把
  `planShards` 的**片边界**（确定性、可复现）升格为增量分类依据——片内批边界
  虽不可复现，但「这一片的内容有没有变」是确定性的，可做片级三态（变动的片
  整片重转、未变的片跳过）。两条都暂不做。
