...
3. 刷卡**不等待 AI**：Leitner 照常即时出结果；AI 重排是异步批量的
   （攒批触发，如会话收尾或攒 N 词），调整在下个会话生效。
...
* `WordStore.ts`：`WenguWordProgress` 增计时段（如 
  ...
  （超时 → 等同 grade "no"）。
* `WordAi.ts`：待分析集合从「只有误认词」扩到「误认词 + 答对但
  持续犹豫的词」；`buildPrompt` 带停留时长/题型/答错历史；
  回复协议与 `applyAiPlan` 按决策 4 改档位制。
* i18n `src/i18n/{zh-CN,en}.json` 同步新文案（若有 UI 变化）。
...
* 刷卡全程无 AI 等待（重排按钮/批量触发与现有 WordAi 交互一致）。
