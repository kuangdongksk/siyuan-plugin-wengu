...
2. 计时只累计**可见且非输入**的时间：`visibilitychange`/窗口隐藏
   时暂停累计（切应用、最小化不冤枉——技术上检测得到）；**输入框
   聚焦/打字期间同样暂停**——打字速度慢不该被当成犹豫，spell 的
   决策信号实际成为「看题到开始输入的首键延迟」；可见、非输入
   状态下的超时（走神，无法与深度思考区分）**按「忘记」处理**。
   用户原话的自愈
...
6. **每组单词数量**：新设置项（默认 10，范围 5~20，存插件数据）。
   ...
   AI 调用通道（20260829 用户修订：弃用 "" 空会话）：走
   `agentChatOnce` 一次性独立会话——独立 sessionID 天然并发，各组
   直接发起互不排队，也不与判分/转换互堵；组大小超过 WordAi 单批
   上限时自动分批（`BATCH_SIZE` 多批顺序执行已支持）。
   （原拍板为「走既有 `enqueue` 串行队列」，随空会话弃用作废。）
7. **误认对（A 认成 B）**：作答中出现「把 A 认成 B」的证据——
   自述框填了 B、拼写题拼成了 B（错结果是真词）、选择题选了
   B 的释义——则**即使该题判对，A 也算没记住**：本组内隔卡
   重现一次（复用 ded9e1b 的错词重现机制），明天到期；若 B 在
   词书内（如 resolution/revolution），B 一并调到明天，A、B
   **对照复习**；B 不在词书内（如 revalue），只在 A 的详情区
   带 B 的辨析对照。AI 组分析负责从整组作答数据里识别这类
   误认（不只依赖自述——拼错结果、被排除的干扰项都是信号），
   返回配对建议。
...
- 内核 agent/chat 并发锁按 sessionID 键控：无 sessionID 的调用共用
  "" 会话必须过 `enqueueAi` 串行队列；一次性任务用 `agentChatOnce`
  （独立 sessionID 天然并发，单词复盘 20260829 起走此）。
