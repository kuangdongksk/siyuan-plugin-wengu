...
3. 图标型入口（如目录底部「设置」）只放图标本体，文字放 title 提示。
   **svgIcon 输出无 width/height，必须落在有 `svg` 尺寸规则的容器里**
   （`wengu-iconbtn`/`b3-button` 等；裸插进普通文本行会按 SVG 替换
   元素默认尺寸渲染——块内铺满容器、行内 300×150，相关题弹窗大放大
   镜即漏加后果，20260901）。新容器加规则统一进 panels.scss
   「全局图标尺寸」表，勿散落单点样式。
