...
3. 图标型入口（如目录底部「设置」）只放图标本体，文字放 title 提示。
   **svgIcon 默认自带 width/height=14 属性（20260905 根修）**——新
   容器直接裸插即安全；需要不同尺寸用 CSS 覆写属性（CSS 优先级高于
   尺寸属性）。此前 svgIcon 不带尺寸、靠 panels.scss「全局图标尺寸」
   容器清单给 14px，新容器漏进清单就按 SVG 替换元素默认尺寸渲染
   （块内铺满容器、行内 300×150），相关题弹窗大放大镜、复习时间线、
   预览搜题框三犯同坑后收口；清单保留为保险层，新规则勿再散落单点。
4. **按钮分级与用色（2026-08-26 补全；20260915 开刷面板例外）**：
    - 主/普通操作：`b3-button--outline`（主题色边框+文字，AI 转习题/
      开始转换/AI 分析报告）
...
6. **间距体系（2026-08-26 起，基数 4px）**：
   **总则：凡横向并排 ≥2 个按钮的行（面板操作行/弹窗底部操作行/浮层
   ...
   gap:8px，别等补救。**唯一显式例外＝开刷面板操作行 14px**（见下表，
   Issue #100 照设计稿屏①）。

    | 位置                              | 间距 | 落点                                                                                                                                                                                                                                                                       |
    | --------------------------------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | 开刷面板操作行（预览/开始/回顾）  | 14px | `wengu-start-actions`——Issue #100 照设计稿屏①，间距随卡片尺寸放宽（**8px 总则的显式例外**）                                                                                                                                                                                |
...
9. **显隐切换禁用「设 display 的类 + hidden 属性」组合（2026-08-26 起，
   网页版复现实锤）**：作者层任何 `display` 声明（如思源
   `.b3-button{display:inline-flex}`、自设 `display:flex` 容器）都会压过
   UA 样式表的 `[hidden]{display:none}`——`hidden` 属性失效、元素照常
   渲染（转换弹窗「终止生成/查看进行中的转换」常驻 hidden 却全部露出
   即此因）。规范：JS 切显隐要么换类名（`.wengu-side-collapsed` 这类
   专用隐藏类），要么元素类**不设 display** 并给 `[hidden]` 补作者层
...
11. **整页不滚动（2026-09-15 起）**：面板高度一律适配宿主视口，滚动收进
    面板**内部的滚动窗**，工作区主区不出现页面级滚动条。
    > ⚠️ **口径变更记录（保留作历史）**：2026-09-15 Issue #129 曾让 AI 会话
    > 面板「撤回收内滚」（按稿的一卡两栏、卡随内容长），2026-09-16 Issue #146
    > 因真机又出全屏滚动条而**恢复本条**（宿主档 + 高度链 + 列内滚窗复位）。
    > **现行口径以 `docs/design-spec.md` §12 为准**；本条只留审查清单。
    - **原则**：面板的「常驻件」（标题栏、说明 hint、过滤条/工具行）与
      「清单头」（在途流横幅等）必须**钉在视野里**——用户滚的是清单与
      详情，不是面板本身。列表超过视口时**长列表列自己滚**，不是整页滚：
      整页滚会把标题、过滤条、横幅一起带走，长清单场景下等于「永远看不
      到头」。
    - **技术要点（缺一条链就整页照滚，表现为内滚窗不出现）**：
        - **flex/`min-height:0` 链要一路打通**：主区 → 面板页根 → 卡 →
          卡内两列，每一级都 `min-height:0`（flex 子项与 grid 子项的自动
          最小尺寸=内容高，缺 `min-height:0` 的那一级会把祖先撑长）；
        - **卡内 row 高必须显式分配**：grid 只给「行高吃剩余」时写着
          紧凑行会按**内容**长（`grid-template-rows: auto minmax(0, 1fr)`，
          或 flex 列 + `flex:1`），否则列没有「剩余高」；
        - **主区容器必须允许收内滚**：工作区主区（`.wengu-ws-main`）自带
          `overflow-y:auto` 是**共用骨架**，其余面板靠它自滚、一个字不能
    ...
    - **落地与例外**：AI 会话工作区（Issue #96）曾按本条改造，但**#129
      按对稿口径撤回**——该面板的稿形态是「一卡两栏、卡随内容长」（gap-list
      S4：卡内两列都不设滚动窗/max-height），与「两列各自内滚」正相反；
      故 `--fit` 档与其判定模块（`ai/core/PanelFit.ts`）整体退役，AI 会话
      面板回到与其余管理面板同款：**外层面板壳（`.wengu-ws-main` 的
      `overflow-y:auto`）就是那扇唯一的滚动窗**，卡随内容长、卡外件与卡内
      两栏同属一卡、一并滚。**其余管理面板（专题/知识/统计/学伴）的迁移
      不在该条生效范围**——规范是总则，存量面板按其自有节奏迁移。

## 一、现状全景

> 下表是 2026-08-24 审查时的快照。2026-08-26 起源码已按功能分域重构
> （`src/wengu/*` → `siyuan/quiz/convert/review/word/stats/bank/ui`，
> 词库数据入 `word/data/`，见 AGENTS.md 项目速览与 CHANGELOG），表中
> 路径/行数仅作历史参照；§〇 的界面规范仍然有效。
