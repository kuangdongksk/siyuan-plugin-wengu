...
export function openWeakDrill(deps: WeakDrillDeps, rows: WeakTopRow[]): void {
    ...
    const dialog = new Dialog({
      ...
      <div style="display:flex;gap:8px;margin-top:8px;align-items:center">
        ...
        <select class="b3-select" data-act="drill-count">${
            [1, 2, 3, 4, 5].map((n, i) => formOption(String(n), String(n), i === 2)).join("")
        }</select>
      </div>
    ...
    root.querySelector("[data-act='drill-ok']")?.addEventListener("click", () => {
        const mode = (root.querySelector<HTMLSelectElement>("[data-act='drill-mode']")?.value ?? "variant") as
            | "variant"
            | "concept";
        const count = Math.min(MAX_PER_RUN, Number(root.querySelector<HTMLSelectElement>("[data-act='drill-count']")?.value ?? 3) || 3);
        void runDrill(deps, rows.filter((r) => selected.has(r.key)), mode, count, show, dialog);
    });
}
...
async function generateOne(
    ...
    if (mode === "variant") {
        ...
        const wrongMost = records.filter((r) => r.stats.wrongCount > 0)
            .sort((a, b) => b.stats.wrongCount - a.stats.wrongCount)[0] ?? records[0];
        template = wrongMost?.kramdown ?? "";
        if (!template) return ""; // 变式必须有真题模板
    }
    const prompt = mode === "variant" ?
