...
export function openConvertDialog(deps: ConvertDialogDeps): void {
    ...
    <div class="b3-dialog__action">
      ...
      <button class="b3-button b3-button--text" data-act="dlg-stop" hidden>${esc(t("convertStop"))}</button>
    ...
    const chooseStop = (r: BatchedResult, srcDocId: string) => {
        ...
        okBtn.onclick = () =>
            void (async () => {
                ...
                setBusy(true);
                try {
                    const created = await writeExerciseDoc(info, r.kramdown);
