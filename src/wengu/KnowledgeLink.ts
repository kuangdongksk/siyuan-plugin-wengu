...
/** 生成 prompt 的第 9 条规则（仅在路由出小节时追加）。 */
export function knowRule(): string {
    return `
9. 知识点标注：文末「知识点清单」列出本批内容可能涉及的知识点（K 编号）。每道题按考查内容在容器属性行末尾追加 custom-plugin-wengu-know="K1,K3"（1~3 个最相关编号，逗号分隔；只能用清单里的编号，不得编造；没有合适的不加该属性）。`;
}
