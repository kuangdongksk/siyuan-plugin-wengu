<script lang="ts">
     ...
     * 不是它的时间），故落点在**未选中**时出记录时刻的弱注记（恒正确），
     * 选中即整行让位给详情。删除钮仍 hover 才显（rail.scss 既有口径），
     * 不占常驻视觉位。
     *
     * ⚠️ **注记只在「未选中且非运行中」时出**（Issue #170，20260918）：
     *  - **运行中行整条不渲染**（判据取**视图**的 `lv.spin`，与徽标转圈同源；
     *    不用 `lv.dotCls === 'run'`——色名是**视图词表**、状态词是数据词表，
     *    两套混用正是 #92 的踩坑）：设计稿叶子行只有「点 + 任务名 + 徽标」
     *    三件，运行中再多一件常驻注记，窄侧栏下第一行就被塞满；要时刻就点开
     *    看详情日志首列（本就「选中即让位」，运行中不过是把这个让位提前）。
     *  - **注记去类别段、只留「MM-DD HH:MM」**：种类优先两级树里叶子已挂在
     *    「转换」组行下，再写「· 转换」是同屏重复；且注记与**贴右**的徽标
     *    同排（`.wengu-aipanel-badge { margin-left:auto }`），串越长越把徽标
     *    往左挤。类别信息由组行表达，注记只答「这批是哪一刻的」。
     *  - 时刻仍取**记录级** `createdAt`（见上段「假时刻」的教训）；
     *    i18n `aiRowMeta` 一段式，调用点只喂 `time`。
     */
                            ...
                            >
                                {#snippet main(n)}
                                    ...
                                    {:else}
                                        ...
                                        <!-- 行尾弱注记（**仅未选中且非运行中**，见头注释「行尾弱注记」）：
                                             只出记录级真实时刻（类别由组行表达，运行中整条让位给三件套） -->
                                        {#if ui.selId !== n.id && !lv?.spin}
