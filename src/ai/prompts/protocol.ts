 ...
 * 20260921 Issue #206 收敛转换链）：
 *   - 缺省（不带 opts）= **正确项写最前**（字母由渲染按序自动编、洗牌
 *     消剧透由代码做）——只留给**新造题**（讲义/笔记出题、变式、概念
 *     辨析/加练）：新造题无原文顺序可保，答案恒 A 便于自检。
 *   - `opts.bank`（Issue #131 引入、Issue #206 收敛为单规则）= 转换链
 *     口径：**照原文抄**——不写字母标签、按原文选项顺序、不重排不增删，
 *     `@@P ans` 按原序数写字母。模型只做 transcription，不做「有无现成
 *     选项」的分类、也不重排（重排类错误在结构上不可能再发生）。
 *   - `opts: "keep"` = 沿用原题顺序与字母（题内 ans/解析的字母引用随之
 *     自洽）：单题修复重生成（变式解析引用模板字母，动不得）。
 *
 ...
 *  字母」（只给造题链）；"keep"=沿用原题顺序与字母（变式解析引用模板
 *  字母，故 regen 链动不得）。转换链走 `opts.bank`（Issue #206）。 */
...
/** 转换链单规则（Issue #206）：纯照抄，零判断零重排。
 *  #131 的「逐题判断有无现成选项」在真机是概率性的（同一卷出两种样式，
 *  甚至两个分支都没干净执行的杂交第三形态），格式层留的发挥空间正落在
 *  「答案字母映射」这个最易错处；且「正确项写最前」对转换链不成立——
 *  选项顺序与答案位置是**源文档事实**，不是可选排列（该口径只对造题链
 *  有意义，见 OPT_LINE_DEFAULT）。故收敛为一条机械规则：模型只做
 *  transcription。字母标签由系统统一编，AI 产物永不出现字母。 */
const OPT_LINE_BY_BANK =
    "选项内容（每个选项一个 @@P opt）——**照原文抄**：只写内容、不写字母标签（字母由系统统一编），按**原文选项顺序**排列，不重排、不增删。@@P ans 按原文选项顺序数：原文答案是第几个选项就写对应字母";
 ...
 *  （转换链口径优先于 regen 的 keep 序）。三个变体的输出差异**仅在 @@P opt
 *  行**（测试按行 diff 锁着）。
 *
