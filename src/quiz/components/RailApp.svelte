<script lang="ts">
     ...
     * 的 mountRailFor）：刷题/知识/AI 会话/学伴四个图标钮（20260831
     * □4 合并：专题管理并入知识）。rail 是「随壳重绘」的一部分——
     * 四处壳拼接（做题主壳/错误兜底/工作区分支/复习分支）都在
     * innerHTML 最前放 RAIL_ANCHOR_HTML 锚，mountRailFor 以 anchor 法
     * 把组件根插到 v.el 直下后删锚（flex 三栏布局依赖直接子元素）。
     */
    let {
        t,
        active,
