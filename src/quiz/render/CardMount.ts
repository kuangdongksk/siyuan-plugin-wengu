...
/**
 * 题卡挂载编排（6-4a 渲染层组件化）：静态分片管线逐单元以组件挂载
 * 替代 insertAdjacentHTML 字符串插入——组件根顶替原字符串输出（追加
 * 到容器尾，DOM 契约逐字一致），renderOneUnitHtml/renderCardHtml/
 * renderSlotsCardHtml 字符串渲染退役。作答/判分仍由三流程直改组件
 * 产出的 DOM（三写收敛为卡内响应态是 6-4b）。
 *
...
export function mountDrillUnit(container: HTMLElement, u: DrillUnit, m: CardHtmlModel): void {
    try {
        if (u.kind === "single") {
            apps.push(mountSvelteApp(QuizCardApp, container, { q: u.q!, idx: u.idx!, m }));
        } else {
            apps.push(mountSvelteApp(GroupUnitApp, container, { qs: u.qs ?? [], mid: u.mid ?? "", t: m.t, m }));
        }
    } catch (e) {
