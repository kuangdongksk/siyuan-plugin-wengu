 ...
 *
 * 匹配文本源 = DOM 侧按跳表收集的文本节点（ClueMarkDom.textNodesOf）：
 * 跳表命中即**整片退出匹配源**，多排除一个类就让含该类文本的选段子串
 * 必败（Issue #51 真根因——`.wengu-gloss-link` 混进跳表即此）；而落格
 * 守卫（`SUP_SELECTOR`）**只挡「不许被包 mark」、不挡匹配**。两者管的事
 * 不同，别混用（本文件只吃节点表，判定全在 DOM 侧）。
 */
 ...
 * 侧按标记元素收集（跳过既有 mark/脚本/词表/解析区等；跳表即匹配源，
 * 见文件头说明）。
 *
