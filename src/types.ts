 ...
 *
 *  **唯一调用点＝落库**（Issue #176 复核实查）：`QuestionDraft.renderUnit`
 *  拼 `- X. ` 前调用（库里本就不该存双字母）。展示层原先在
 *  `ProtyleHost.optionRowHtml` / `CardState.optSnaps` 另叠一层作兜底的写法
 *  已删（Issue #214）——它只对 ≥4 层畸形存量有效，存量兼容口径已整体退役。
