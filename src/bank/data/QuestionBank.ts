...
export interface BankSet {
     ...
     *  解析。
     *  **唯一消费点 = 语言专属功能「标生词」**（quiz/flow/AnnoScope 的
     *  isEnglishScope）：题型是**作答形态**、不是学科——「英语阅读的
     *  single 与数学单选都是 single」「语文的 essay/trans 也在英语四类
     *  形态里」，判语言只能看真实学科（无本字段的存量题集回退题型并集）。
     *  ⚠️ **不许拿它当任何视觉/结构的判据**：阅读面是**材料组结构**判据、
     *  与学科无关（见 quiz/flow/ReadingScope，#83 根因即「阅读面绑英语
     *  判别」修错方向）。
     ...
     *  optional、只加不改名、不 bump version；无此字段的存量题集 = 现状
     *  行为（提示 + 整卷重转，不认整篇哈希）。 */
