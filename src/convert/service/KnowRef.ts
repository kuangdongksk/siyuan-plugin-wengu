 ...
 * ——转换注入（applyKnowLinks 把容器 know 别名映射回真实块 id）与
 * 事后匹配/标签落库（strip+inject 的替换语义，KnowLinkText 共用）同走
 * 这一份格式；sectionKramdown 供单题重生成喂小节正文。纯文本与 SQL，
 * 无 AI 调用。
 */
...
/** 生成「相关知识点」引用行（转换注入/事后匹配共用，反链面板可见的
 *  同格式）。 */
