 ...
 *
 * ⚠️ **本模块只出判据、只服务落库链**（Issue #176 收窄，20260919）：出口就
 * 下面这四件（`isRefLetter`/`LETTER_TOKEN`/`protectionMask`/`OPEN_BEFORE`）。
 * 渲染路径不消费它——洗牌**不对用户文本猜字母**，存量的裸字母记录由用户
 * 重新转换消化。要加出口先问「落库链真的需要吗」。
 *
