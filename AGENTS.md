...
- **选项字母标签剥层口径分两侧**（Issue #176，20260918）：
  ...
  **落库/规范化**（`types.normalizeOptionLabels`，默认 2 层）给「标签 + 正文」用。
  **主修落点＝落库**：`convert/service/draft/QuestionDraft.renderUnit` 拼
  `- X. ` 前调用（库里本就不该存双字母 `A. A. 时空…`）。
  **展示侧兜底**在 `quiz/service/ProtyleHost.optionRowHtml` 与
  `quiz/render/CardState.optSnaps`——⚠️ 对真机常见双字母是**空操作**
  （#163 起 `optionDisplayMd` 已连续剥层封顶 3），别当主因。
  改任一侧先读 `.agents/memory/convert.md`（#163 双标签实测口径）。
...
- **整页不滚动**（Issue #96，2026-09-15 起硬性规范，见 `docs/design-spec.md` §12
  ...
  ⚠️ **AI 会话面板的例外登记已于 20260916 撤销**（Issue #146）：`#129` 曾按
  gap-list S4 把它改成「单滚动窗」（卡随内容长、滚动归外层 `.wengu-ws-main`），
  `--fit` 档与 `ai/core/PanelFit.ts` 一并退役 —— 结果是本条规范**被打破**
  （真机走查又出全屏滚动条）。现恢复 #96 的高度链：宿主档
  （`ai/core/PanelFit.ts` 的 `workspaceFits`/`fitTargetOf`/`toggleFit`）+ 挂载/
  卸载配对开关（`ai/SessionPanel.ts`）+ 卡内两列各自内滚窗，而 **#129 的视觉
  逐值保留**（一体卡/树/详情/横幅/轮次日志块，只改「高度从哪来、滚在哪滚」）。
  **判据看稿不看规范条文**：被设计稿明确约束的面板按稿落形态并登记例外；但
  「稿的两栏固定宽度档」不等于「可以让页面级滚动条回来」。**其余管理面板
  （专题/知识/统计/学伴）的迁移不在本规范当前的改造范围内**（规范是总则，
  存量面板自行排期）。
