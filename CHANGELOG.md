...
- **题集补真实学科：英语卷判别不再拿题型当代理**（20260914，quiz / bank / convert
  域，Issue #83）：阅读面（#81）与浮条「标生词」（#45）共用的英语卷判据原先只有
  一条腿——「该题集**题型并集**含英语四类（cloze/match/essay/trans）任一」。但
  题型是**作答形态**不是学科，代理两个方向都会判错：**纯阅读英语训练卷**（题目
  全是单选）判非英语 ⇒ 漏给阅读面、正文浮条不出「标生词」；**语文卷**含作文
  （essay）与文言文翻译（trans）⇒ 误判英语卷，中文词被收进英文生词本。

    - **题集新增 `subject` 字段**（真实学科）：转换**首批判定行**顺带报出
      （`SUBJECT:` 行，与 CAN_CONVERT/REASON/TYPES 同行协议、零额外 AI 调用，
      非首批不报），随题集落库；optional、只加不改名、不 bump version，
      存量题集**零迁移**。
    - **判定改两级**（`AnnoScope.isEnglishScope`，唯一判定点）：**有学科以学科
      为准**、**无学科回退题型并集**（存量题集逐字节不回归）。学科归一**只去
      装饰不做模糊匹配**（学科是开放集，猜错比不猜更坏）：写「英语（阅读理解）」
      取「英语」，写「无/未知/N/A」按无学科收口。
    - **混合刷按题集段各判各段**：聚合「全部习题」/跨学科专题下，整壳类名只在
      **全段皆英语**时挂；英语段套一层 `.wengu-set-seg.wengu-reading` 段作用域
      （标题行留在包装外，题集标题行的首/续段间距口径不变），数学段既不挂类
      也不多套 DOM。单题集（一段）渲染产物**逐字节不变**。
    - 浮条「标生词」的反查链（按卡所在卷判、按题集缓存）天然按段，无需改动。
