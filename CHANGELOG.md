...
- **修复：移动端「继续上次」三缺口——跨题集探测不到、count 轮恢复展开全量、
  ...
  开始」。
    ...
    - **A1 探测改全库**：原实现恒取激活题集（`sets[0]`）的最后一条会话，
      ...
      **最近一条**未完成轮（判据不变：`answered > 0 && !endedAt`），激活卷没有
      可继续的轮就跨到最近那条所在的卷并顺手切好卷面；恢复卡显示**目标题集
      标题**与已答数。点击恢复走新入口 `resumeRound()`：跨题集时先装载目标
      题集，再**以恢复卡携带的会话对象直接恢复**，恢复路径不二次探测。
    ...
    - **A3 恢复落点**：`start("continue")` 恢复后 `qIdx` 落到第一道未作答题；
      答满未交卷的轮维持第 1 题。
    - **顺带修掉一处真实数据丢失**：恢复路径原先会重放一次
      `history.upsert(session)`，而 upsert 是「整段换对象」语义——它会把这条
      刚读出来的**同 id 记录覆盖掉**，其余轮次随之消失（真库上最直接的后果
      是阻塞全库探测的其他题集未完成轮被抹）。恢复路径改为不回写，会话即
      权威现场。
    ...
    - 用例新增 `mobile/core/MobileDrillResume.test.ts`（A1/A1b 两边界/A2 含
      排列一致/A3 两例，7 例）；夹具按域拆片口径拆出
