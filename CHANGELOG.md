...
- **修复：AI 会话面板叶子行注记收窄——运行中不渲染、去类别段只留时刻**
  ...
  三件——`wengu-aipanel-meta` 是 #129 的**间隙期形态**（登记簿缺逐轮时刻，
  先以记录级 `createdAt` + 类别章过渡），叠加在常驻徽标之后就成了第四件。

    - **运行中行整条不渲染注记**（判据取**视图**的 `lv.spin`，与徽标转圈同源）。
      刻意**不**用 `lv.dotCls === "run"`（色名＝视图词表）或
      `rec.status === "running"`（状态词＝数据词表）：#92 的踩坑正是两套词表
      混用——只有 `done` 重叠，`run` 会静默失效。要看时刻就点开看详情日志首列
      （本就「选中即让位」，运行中只是把这个让位提前）。
    - **其余行注记去类别段、只留「MM-DD HH:MM」**：种类优先两级树里叶子已挂在
      「转换」组行下，`· 转换`是同屏重复；且注记与**贴右**徽标同排
      （`.wengu-aipanel-badge { margin-left: auto }`），串越长越把徽标往左挤。
      时刻仍取**记录级** `createdAt`（保住「假时刻」教训）。
    - **i18n 与规格锁**：`aiRowMeta` 中英同改一段式 `"{time}"`，调用点只喂
      `time`；`AiPanelGapRestore.test.ts` 新增「叶行注记形态」源级锁 3 条
      （运行中不出且判据不许拿色名/状态词、模板与调用点双双无 `{kind}`、
...
- **修复：移动端「继续上次」三缺口——跨题集探测不到、count 轮恢复展开全量、
  ...
  恢复卡 + 探测）本身存在，但三条缺口让它形同虚设，用户感知为「每次都是新的
  开始」。

    - **A1 探测改全库**：原实现恒取激活题集（`sets[0]`）的最后一条会话，
      未完成轮不在首个题集时（多套题常态）恢复卡完全不出现。现扫全库取
      **最近一条**未完成轮（判据不变：`answered > 0 && !endedAt`），恢复卡
      显示**目标题集标题**与已答数（分母按目标题集算）。点击恢复走新入口
      `resumeRound()`：跨题集时先装载目标题集，再**以恢复卡携带的会话对象
      直接恢复**，恢复路径不二次探测。**探测只读**——绝不在探测里切卷面
      （那会让开刷面板的题集行点不动、连「返回题集」也被弹走），切卷只在用户
      点恢复卡那一步发生；目标题集在探测后被删则清卡不开空卷。
    - **A1b 边界**：目标题集存在**更新的已收卷轮**时，取「最后一条」的旧口径
      会把它挤掉；全库探测只收未完成轮即不受影响，且恢复路径不重探测——
      「继续」不会退化成新开。题集已删（不在 sets 清单）时不显示恢复卡。
    - **A2 fresh 轮写 scopeIds 快照**：仅当「本次题数」真的裁掉题时写
      `scopeIds: list.map(q => q.id)`，全量开轮不写（与桌面 `scope === "all"`
      不写同口径）。存量无快照的轮行为不变（全量兜底），零迁移、不 bump version。
      「选 20 题答 8 题退出」现恢复为同一子序列，排列也与退出前一致。
    - **A3 恢复落点**：`start("continue")` 恢复后 `qIdx` 落到第一道未作答题；
      答满未交卷的轮维持第 1 题。
    - **恢复路径不再回写会话**：原先会重放一次 `history.upsert(session)`，
      而 upsert 是「按 id 整段换对象」——重放一份刚读出来的旧快照会把同 id
      那条的**新态盖回去**（陈旧覆盖），且恢复本身零字段变化、白写一遍整文件。
      会话已是权威现场 ⇒ 只有 fresh 开轮才 upsert。
    - **落点**：恢复/开轮/关轮生命周期整体落 `mobile/core/MobileRound.ts`
      （`MobileDrill.ts` 余量仅 9 行，只留薄转发 `restoreResumeFor` /
      `start` / `resumeRound` / `retryWrong` / `initRoundCards`）；
      「未完成轮」判据收口为 `MobileRound.isUnfinishedSession` 单一实现。
    - 用例新增 `mobile/core/MobileDrillResume.test.ts`（A1 跨卷探测与点击恢复、
      探测只读不劫持选卷、A1b 两边界（更新的已收卷轮 / 题集已删）、A2 含排列
      一致、A3 两例，9 例）；夹具按域拆片口径拆出
      `mobile/core/MobileResumeMock.ts`，`MobileDrill.test.ts` 回到 500 红线内
      （610 → 470 行）。i18n 零新键（标题直接用题集标题字符串）、样式零新增
      （复用既有 `.wengu-md-resume` 结构）。
...
- **桌面开刷面板同步设计稿屏①：双卡片 + 唯一主操作 + 统计条分段**（20260915，
  ...
  三个动作按钮同级无主次、无错题时「进度与范围」整卡消失、头部统计条全灰无
  分段。**纯展示层改造，零数据 / 零开轮逻辑改动**。

    - **双卡片结构，范围行恒渲染**：`.wengu-start` 去外盒改居中列容器
      （`min(760px,100%)`、gap 18px），「进度与范围」/「作答设置」成两张独立
      卡片，卡头**在卡内**（小图标 + 15px/600 标题）。**「刷题范围」行无条件
      渲染**（最少只有「全部题目」一项）——旧条件是「未完成轮 **或** 有错题才出
      整卡」，无错题时整卡连同范围行一起消失；「上次进度（继续上次）」维持
      按未完成轮条件渲染（逻辑口径不动）。卡内行内缩归零 + 改纵向内衬（主题
      `.b3-label` 自带 24px 水平内边距会与卡内衬叠加），行间分隔线、末行无；
      控件最小宽 300px（替换 `fn__size200` 的 200px）。
    - **「开始刷题」是唯一主操作**：改 `variant="primary"`，「预览」「错题回顾」
      维持 outline，动作行居中（`gap: 14px`、按钮 `min-width:132px`）。
      ⚠️ 14px 与 §〇6「横向按钮行一律 8px」冲突——**以设计稿为准**，代码注释里
      写明这是本单显式例外，不留自相矛盾的注释。
    - **头部统计条结构化分段**（`CardHtml.renderSubheadHtml`）：段序 = 题集名 →
      已刷/答对 → 竖线 → 轮次成绩；数字加重 + `tabular-nums`（题集名段整段
      加重且**整段逐字转义**，标题里带数字不被误加重）；**竖线只在两侧都有内容
      时才插**，无进度 / 无轮次时不再留悬挂分隔符。文案仍逐字来自既有 i18n 键。
    - **颜色全走 b3 令牌**（Neo 的 `#c5866a`/`#364852` 只作观感参照，零硬编码）。
      新测试 `SubheadHtml.test.ts`（分段/分隔符四态、数字加重、标题逐字转义、
      i18n 键 zh/en 两字典齐备、组件源级结构断言）锁口径；`StartPanel.test.ts`
      零改动，移动端走 `src/mobile/` 自有面板、零影响。
    - **补齐屏①行排版 + 样式拆片**（20260915 续）：`.sr-t`/`.sr-d` 的排版此前
      漏做——卡内行的左标题改 **13.5px/600**、描述 **12px 次要色 + 2px 上留白**，
      卡头间距照 `.scard-h` 的 **gap 9px / 下留白 6px**（原 6px/12px）；末行只去
      分隔线、保留纵向内衬（卡内衬 22 + 行 13 = 稿里 35px 底留白）。
      ⚠️ `.wengu-start*` **自 `panels.scss` 拆到 `src/scss/startpanel.scss`**
      （原文件被本单 +117 行顶穿 500 行红线），`index.scss` 注册；规格断言
      `StartPanelStyle.test.ts` 走 **sass 真编译**钉在这一片上（760px/22px 26px/
      18px、卡头 15px 600、行 13.5px+12px、300px 控件、14px 动作行/132px 钮，
      并断言**无字面色值**——颜色零硬编码这条由测试兜住）。
      行内排版按结构定位（`> .fn__flex-1` / `.b3-label__text`）——FormRow 的
      类名串是主题对抗的一部分、不许加专属类。

- **AI 会话详情：默认展开轮次、输入输出分块、空脚不渲染**（20260915，
  ai / scss 域，Issue #98）：已完成记录（如「生成第 4 批 · 8 题」）的详情右栏
  三处观感问题——一轮也要再点一下才看到产出、几 KB 的 prompt 与回复糊在同一个
  `<pre>` 里、无归属备注时下方空出一条色带。

    - **轮次日志默认展开**：详情的展开集合改为「被显式**收起**的行」（`closedRows`），
      重挂即清空 ⇒ 新记录回到全展开；多轮也全部展开（详情列已有内滚兜底）。点行头
      收起/展开的交互不变——只有默认态变了。开合与可展开两条判据一并落在 core 侧
      （`isRowOpen` / `canExpandRow`，带单测）：组件自持的 `$state` 挂不进
      vitest，判据留在组件里「默认展开」就没有回归锁。
    - **展开后按侧别分块**：user 侧=「输入」块、ai 侧=「输出」块，各带 faint 档
      小标签（**新 i18n 键** `aiLogIn`/`aiLogOut`，中英同步加），缺侧的尾轮只出
      一块；输出块不设 max-height（面板核心用途是回看产出），滚动交详情列的内滚窗。
      **摘要行本身不动**（仍是一行一轮的 S6 形态），分块只在**行展开态**里做。
      块是行内顺序内容、不做卡片嵌套；块间收紧只用相邻块选择器（`+ .wengu-aipanel-logseg`，
      **不能用 `:last-child`**：单块行里首块同时是末块，会把那块的留白一并收掉），
      DOM 序保持真实先后。
    - **空脚不渲染**：`ownNote` 为空且 `retryable` 为假时 `.wengu-aipanel-dfoot`
      整块不出，连带 border-top/底色一并消失；在途/停止态归属备注与真失败重试钮照旧。
    - 视图模型仍全在 `ai/core/SessionDetail`（纯逻辑带单测）：`SessionLogRow.segments`
      随行带回分块正文、标签取词在 core 侧，组件零判断零取词。

- **AI 会话工作区整页不滚动：滚动收进卡内两列，规范补第 11 条**（20260915，
  ai / scss 域，Issue #96）：记录一多就出**整页滚动条**——标题、hint、kinds
  过滤条、在途流横幅全随页滚走，树列与详情失去固定视野（根因是 #93 照
  `design/aipanel-gap-list.md` S4「滚动交宿主页」的取舍，与产品期望冲突，
  本单推翻它）。

    - **整页不滚动成为硬性规范**（`docs/design-review.md` §〇 **第 11 条**）：
      面板高度一律适配宿主视口，滚动收进面板**内部的滚动窗**；常驻件（标题/
      hint/过滤条）与清单头（横幅）必须钉在视野里。技术要点＝flex/`min-height:0`
      链一路打通 + **grid 行高显式分配**（`auto minmax(0,1fr)`，否则行按内容长、
      列没有「剩余高」）+ 主区经 `.wengu-ws-main--fit` 一档改写 + 内滚窗落在
      **列**上 + 列上留 `scrollbar-gutter: stable`（免得有/无滚动条两态行宽跳动）。
      弹窗长内容同口径（容器封顶 + 内容区自滚）。
    - **AI 会话面板落地**：主区骨架 `.wengu-ws-main` 的 `overflow-y:auto`（其余
      工作区共用）**一条不动**，收内滚者经 `--fit` 档改 flex 列 + `overflow:hidden`
      ——高度链才有起点（主区可滚 = 卡永远拿不到剩余高，内滚窗不出现、整页照滚）。
      档位开关**配对**在 `ai/SessionPanel.ts` 的挂载/卸载（主区容器是共享骨架、
      随整壳重建，漏关会让下一块面板继承本档 ⇒ 内容被切且滚不到），判定在
      `ai/core/PanelFit.ts`（纯逻辑带单测）。卡 `.wengu-aipanel` 限高
      （`flex:1 1 auto; min-height:0`）+ `overflow:hidden`；树列与详情列各自
      `overflow-y:auto; min-height:0`，卡外件与卡首横幅常驻视野。
    - ≤1000px 折单列时行高分配同步改（沿用「末行吃剩余」会把树行压成 0 高、
      清单整片消失），树列给 `max-height:40vh` 兜底。
    - **档位开关只动本面板那一份骨架**（20260915 复审修正）：初版用 document 级
      `querySelectorAll(".wengu-ws-main")` 开关档位，而该主区是**共用骨架**——同一
      文档里可以有多份（另一个温故页签、后续新增的收内滚面板），全局选择会把
      **别人的**主区一并改成 flex 列 + `overflow:hidden`（那块面板不收内滚 ⇒ 内容
      被切且滚不到），卸载的 `false` 同理会误关别人的档。现由纯函数
      `ai/core/PanelFit.fitTargetOf(root)` 定目标（宿主自身即主区时就是它，否则只在
      **宿主子树内**下探一次、找不到就零动作），挂载时记下、卸载按同一份收起；
      `PanelFit.test` 用假体记账把「旧写法会把别人的一份也打开」锁成回归项。
    - **aipanel.scss 按 ≤500 行硬约束拆片**：补高度链（#96）与详情改造（#98）后
      该片到 576 行，把「② 树」整段单开 `scss/aipanel-tree.scss`（口径同 rail.scss
      逼近红线时拆出 aiflow.scss 的先例；拆的是文件边界不是职责，类名仍
      `.wengu-aipanel-*`）。新片在 `index.scss` 里 @use 于 aipanel.scss **之后**——
      片内含 ≤1000px 折单列的 `@media`，要覆写基础片里的 `grid-template-*`，而
      `@media` 不加特异性、按源码序决胜（顺序调换窄窗口折单列即失效）。
      已核等价：编译产物去注释后**逐属性胜者相同**（含 `@media` 内窄屏口径），
      即零视觉影响。
    - **范围外**：其余管理面板（专题/知识/统计/学伴）的迁移不在该条生效范围，
      规范是总则、存量面板按其自有节奏迁。

- **阅读面改材料组结构判据，全学科一致美化**（20260914，quiz / bank / convert
  域，Issue #83）：阅读面（凹槽阅读栏 + 衬线正文 + ¶ 段落序号 + 题卡间距阶梯，
  #81）原先挂在「英语卷判别」上，而**判别不出英语时材料组结构还在、美化却没了**
  ——这是 #81 修错方向的地方。**材料组（材料块 + 依附小题 = 一题多问）是全学科
  通用结构**：英语阅读/完形、语文文言文、政治材料分析、工科大题都产出材料组。

    - **组单元无条件美化**（`GroupUnitApp` 自判）：组件本身**就是**材料组，一律挂
      `.wengu-reading`，零学科依赖、零壳层传值（不再消费 `m.reading`）。
    - **判据落在单元上**（`ReadingScope.isReadingUnit`，纯逻辑带单测）：全部
      单元都是材料组 ⇒ 挂整壳类名（产物与改造前同形）；混合（聚合「全部习题」/
      跨学科专题，或工科卷同卷既有独立题又一题多问）下不挂整壳，改由
      `wrapPlanOf(units, segOf)` 出逐单元包装计划（`.wengu-set-seg.wengu-reading`，
      标题行留在包装外、题集标题行首/续段间距口径不变）——**独立题卡始终零装饰**
      （阅读面样式全是后代选择器），不再按首题一判到底。复用条件 = **连续
      且同段**：跨段复用会让第二套的标题行落在包装之后、该段首题反而被塞进
      包装（跑自己标题行上面），单测锁死。纯独立题卷既不挂类也不多套 DOM
      （零回归）。
    - **「标生词」才是语言专属功能**（#45）：判据换成**真实学科**——题集新增
      `subject` 字段（转换**首批判定行**顺带报出 `SUBJECT:` 行，与
      CAN_CONVERT/REASON/TYPES 同行协议、零额外 AI 调用，非首批不报），optional、
      只加不改名、不 bump version，存量题集**零迁移**；`AnnoScope.isEnglishScope`
      两级口径（**有学科以学科为准、无学科回退题型并集**），学科归一**只去装饰
      不做模糊匹配**（「英语（阅读理解）」取「英语」，「无/未知/N/A」按无学科）。
      纯阅读英语卷（single-only）靠 subject 判得出，语文卷的 essay/trans 不再
      误判英语。⚠️ 取用层（`service/AnnoScopeCtl`）的**异步补正腿**里学科须在
      await 题集查询**之后**再窥视（补正分支的前提就是题库未装载，提前取恒
      undefined ⇒ 带学科的纯阅读英语卷落成「回退题型并集」判否、缓存一直错），
      回归测试锁死。

- **AI 会话面板流级横幅：整批停止有唯一入口**（20260914，ai 域，Issue #77）：
  「整批停止」原先藏在 AI 会话面板的**单条记录详情**里——但一条记录只是
  一次调用，用户想停的是一整条流。现在面板**顶部**出「运行中的 AI 流」横幅
  （流名 + 进度摘要 + 停止钮），记录详情**不再渲染停止钮**。

    - **横幅是 ai 域通用注册表**（`ai/core/FlowRegistry`），非转换专属：
      覆盖**转换族**（单篇 / 批量队列 / 增量重转换 / AI 索引）与**六个批流**
      （匹配 / 批量关联 / 生成标签 / 变式重练 / 薄弱加练 / 收集补题）。
    - **转换流停下转「待抉择」**：横幅上直接给「保留已生成 / 全部丢弃」，
      停止与收口都在面板完成，**不强制跳回页签**；页内转换条保持现状，
      两处**同一份文案函数**（不会出现两套进度数字）。
    - **批量队列附六态计数**（完成 / 跳过 / 中断 / 失败 / 取消）。
    - **记录详情换归属说明行**：指向上方横幅；**判分 / 伴学等单调用流不出
      任何停止 UI**（它们本就没有有效的流级停止面）。
    - **停止两击确认**（首击变「再击确认停止」、3s 复位、不上模态框），
      横幅与**页内转换条的停止钮同款**。
    - 颜色一律思源主题令牌**全名**，明暗与第三方主题自适应。

- **线索 mark 主题多色：标线索可自选颜色**（20260913，quiz 域，Issue #57）：
  线索高亮此前只有一种主题色，同一题多条线索在正文里分不开。现在标线索时
  可**自选颜色**——四色取自思源主题的卡片色变量**全名**
  （`--b3-card-<kind>-background` 背景 + `--b3-card-<kind>-color` 前景；裸
  `--b3-card-<kind>` 不存在），明暗主题与第三方主题（Neo）实时适配，零配置。

    - **浮条**：「标为线索」主钮**保持一步标默认黄**（既有交互零回归），
      紧邻的小色块角标开**竖排色板**（新独立积木 `ui/ColorMenu.svelte`，
      零业务依赖、可复用）；选色路径 2 击（开色板 + 点色块）。
    - **chips**：每条线索前缀**同色圆点**呼应正文 mark；**点圆点给该条改色**
      （同一份色板，当前色打勾）。
    - **存储**：会话 `clueColors[qid]: number[]` —— `clues` 的第三个平行
      数组（`-1`=默认黄、`0..3`=四色序），下标严格对齐、optional、不 bump
      version、渲染不回写；删除线索同下标同步删，旧会话无此键 ⇒ 全默认黄
      （存量线索零迁移）。
    - **重叠合并归属**（与 #56 联动）：合并后的 mark 与 chips 取**区间最长
      那条**线索的选色（`MarkSlot.clue` 与 `text` 同源取最长）。
    - **降级**：主题变量取不到时不写内联 style（落回 scss 默认黄）——写一个
      解不出的 `var()` 会让背景整条失效。
    - 顺带按 500 行红线拆分：`MaterialDecorate`（装饰编排）只留 ①~⑤，
      线索施工移入 `ClueDecorate`、DOM 观测移入 `CanonDom`（两侧不再互相
      import，防循环依赖）。
...
- **设置入口挪 rail**（20260910，quiz 域）：设置钮自刷题侧栏头部挪到
  ...
  弹窗不切工作区、无激活态；侧栏头部只剩刷新与折叠，SideMount 的
  settings 分派与 hasSettings 管线随之删除。

- **索引归纳 prompt 判别规则收紧**（20260909，convert 域）：真机踩坑
  自控原理绪论——源文档「发展史」一节的流派名词云（现代控制理论 12
  个分支、智能控制 5 个分支，零讲解共 620 字）被旧规则 3「穷尽方法与
  解法」推着全量铺成 18 个 h3 知识点，占全树近半且进路由索引/文本关联
  词表污染匹配。prompt 三处收紧：穷尽对象加「有实质讲解」门槛（名词
  罗列/发展史/课程框架至多合并一个条目）、新增做题价值锚定（不出题的
  背景性内容不进树）、h1 禁起比本章宽泛的学科名（「自动控制的一般
  概念」→「自动控制理论基础」类大一统标题）。存量树两击重跑即按新
  prompt 重归纳（id 同路径复用不悬空）。
...
- **路由②知识点清单剥公共前缀 + 预算放宽（没带上全部知识点根治）**
  ...
  ~28 字/条）后高数单章 43 条截 1 条、线代 4 章并集 154 条只装 79 条
  ——被截知识点 AI 根本看不到，永远路由不到。修复：新增
  `commonDirPrefix`（段对齐最长公共前缀，条目恰等于前缀时回退一段保
  剥后非空）把书/章路径从清单剥出（prompt 注记省略的前缀值供人核
  对），预算 2200→4500（超时按 SSE 空闲计，5000 字转换批先例）；
  截断兜底保留且编号映射改按截后 kept 集合（不悬空）。转换/匹配/
  批量关联/生成标签四路由入口同享；路由缓存 bump 代数 R2（AI 可见
  小节集合变大，旧缓存结果不代表新路由分布，整表作废——宁漏勿错
  口径）。补 11 例单测。

- **索引树头部章节名回声剔除**（20260908，bank/convert 域）：AI 归纳常
  把章节名本身写成首个 h1 包住全树（prompt 明令禁止仍违逆，真机两棵树
  均中雷）——面板呈现「1-行列式/行列式/…」双层嵌套、回声标题还会混进
  文本关联词表。新增 `stripChapterEcho`（标题去编号归一——「1-行列式」/
  「四、分块矩阵」→「行列式」/「分块矩阵」，中文数字必须带分隔符防
  「一维随机变量」误剥；只剔头部连续 level-1 同名节点）：生成侧入库前
  剔 + 重索引旧树同口径对齐（同路径复用旧 id 不悬空），展示侧 treeHeads
  带章节名同剔——存量带回声的树读取时免迁移。补 6 例单测。
...
- **「建知识树」改名「索引」，重新索引两击确认**（20260908，bank/ai 域）：
  ...
  「移除」两击模式），确认后才重跑（同路径复用旧 id 的语义不变）；
  运行中「索引中…」再点=中止照旧。入口同步放开：撤销旧「结构单薄」
  门槛（小节 <6 且顶层 <3 才显示），全部手动导入文档行常显——多节章
  （如 2-矩阵 20 节/3-向量 9 节）原先整行没有索引按钮。文件夹式文档
  （思源文档当目录用，自身空、子文档有内容）点「索引」=批量补齐子树
  缺索引的文档：串行逐篇、空文档跳过、部分失败不打断、终态汇总通知；
  批量也走两击确认（「确认索引 N 篇」）；已索引的不动，重索单篇走行内。
  顺修 AI 会话
  面板 outline 类别缺 KIND_KEYS 映射一直裸显英文 "outline" 的旧账
  （补 aiKindOutline=索引）；通知/失败文案统一「索引」口径（track
  标题同步）。

- **全 AI 弹窗去阻塞：点击即关窗、后台流、面板停止**（20260905，ai/
  bank 域）：七个 AI 弹窗（重新生成/匹配/批量关联/生成标签/变式重练/
  薄弱加练/收集补题）统一「点击即关窗」——参数收齐即销毁弹窗、AI
  后台跑，进度=AI 会话面板 track 实时登记（一次调用一行 running→done），
  终态走思源通知；变式重练完成照旧自动切专题，收集补题的纯收集支路
  不变。机制件：`ai/flow.ts launchAiFlow` 单飞闸——弹窗模态原是天然
  串行闸，后台化后必须自建（匹配/标签的引用注入走内核写流，并发互吞
  响应），六重型批流同一时间只放一条；`client.ts` 中止登记簿
  （`aiAbort()` 句柄 + `AiTrack.onSid` 运行时回调把面板记录 id 挂回
  流级 AbortController，调用收口自动注销）——AI 会话面板 running 记录
  新增「停止」钮（`abortAiSession`，中止保留已产出部分不清偿；重新
  生成也接此通道，原先 X 掉弹窗 AI 照跑）；GenCore/GenQuestion 补
  AiAbort 通道。转换/增量自带页内转换条与停止钮，有意不接面板停止
  （点停静默无效属预期）。清理弹窗内转圈的六个死 i18n 键，补 3 例
  中止接线/单飞闸单测。
- **选项挤行拆行修复 + 题库体检入口**（20260905，convert/bank 域）：
  AI 无视行协议「每个选项一个 @@P opt」把全部选项一行一个塞进同一
  部件时，renderUnit 只给首行编 `- A.` 字母、其余行成续行——反解侧
  只剩 1 个选项，且协议规定正确项写最前恰在首行，即「只剩正确选项」
  （真机概率篇 45 题中 21 题；洗牌 n<2 跳过、答案字母口径 AI 混用
  原卷字母与重排字母均不可信）。双件修复：生成侧
  `OptionShuffle.unpackPackedSingle`（洗牌入口前置拆行、答案按
  「首行=正确项」重写 A 再洗牌消剧透；multiple/steps 步组正确集合
  规模不可推导不拆，保持可检出形态），四个生成入口一处收口；存量
  清偿 `bank/data/BankRepair` + 专题工作区「题库体检」入口（全库扫描
  →摘要+逐题预览（拆出选项/拟答字母/解析提及字母）→勾选确定性修复，
  `replaceRecordKramdown` 原题位回写、qid/题集/作答统计/容器 IAL
  （含 src-key/src-hash）逐字节不动、指纹自动重算，预览即所得；多选
  挤行只报告走单题重生成）。离线用真实损坏数据验证 20 可修/1 多选/
  24 健康全部判分闭环。
